...
router.get('/:id/pages', auth, async (req, res) => {
    try {
        ...
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权访问' });
...
router.post('/:id/pages', auth, async (req, res) => {
    try {
        ...
        const work = await Work.findById(req.params.id);
        if (!work) return res.status(404).json({ message: '作品未找到' });
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权修改' });

        work.pages.push({ title, content });
        await work.save();
        res.status(201).json(work.pages[work.pages.length - 1]);
    } catch (error) {
        res.status(500).json({ message: '添加页面失败', error: error.message });
    }
});

// 更新页面
router.put('/:workId/pages/:pageId', auth, async (req, res) => {
    try {
        const work = await Work.findById(req.params.workId);
        if (!work) return res.status(404).json({ message: '作品未找到' });
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权修改' });

        const page = work.pages.id(req.params.pageId);
        if (!page) return res.status(404).json({ message: '页面未找到' });
...
// 删除页面
router.delete('/:workId/pages/:pageId', auth, async (req, res) => {
    try {
        const work = await Work.findById(req.params.workId);
        if (!work) return res.status(404).json({ message: '作品未找到' });
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权修改' });

        work.pages.pull({ _id: req.params.pageId });
        await work.save();
...
router.post('/:id/chapters', auth, async (req, res) => {
    try {
        ...
        work.chapters.push({ title, pages: [] });
        await work.save();
        res.status(201).json(work.chapters[work.chapters.length - 1]);
    } catch (error) {
...
// 更新章节
router.put('/:workId/chapters/:chapterId', auth, async (req, res) => {
    try {
        const work = await Work.findById(req.params.workId);
        if (!work) return res.status(404).json({ message: '作品未找到' });
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权修改' });
        ...
        if (!chapter) return res.status(404).json({ message: '章节未找到' });
...
// 删除章节
router.delete('/:workId/chapters/:chapterId', auth, async (req, res) => {
    try {
        const work = await Work.findById(req.params.workId);
        if (!work) return res.status(404).json({ message: '作品未找到' });
        if (work.author.toString() !== req.userId) return res.status(403).json({ message: '无权修改' });
